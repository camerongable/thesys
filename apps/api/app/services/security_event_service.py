...
_CROSS_PROJECT_DENIAL_EVENT = "cross_project_access_denied"
...
def record_security_event(
    ...
    *,
    ...
    _detect_cross_project_enumeration(db, event, settings or get_settings())
...
def _detect_cross_project_enumeration(
    db: Session,
    event: SecurityEvent,
    settings: Settings,
) -> None:
    if event.event_type != _CROSS_PROJECT_DENIAL_EVENT or event.source != "auth":
        return
    ...
    matching_events = [
        SecurityEvent.workspace_id == event.workspace_id,
        SecurityEvent.event_type == _CROSS_PROJECT_DENIAL_EVENT,
    ...
    if (
        observed_count is None
        or observed_count < settings.security_cross_project_enumeration_threshold
    ):
        return
    ...
    record_security_event(
        ...
        langsmith_trace_id=event.langsmith_trace_id,
        temporal_workflow_id=event.temporal_workflow_id,
        event_type="cross_project_enumeration_attempts",
        severity="high",
        source="auth",
        summary="Cross-project access denials exceeded the configured threshold.",
        attributes={
            "observed_count": observed_count,
            "window_seconds": settings.security_alert_detection_window_seconds,
        },

...
class GroundedGuideAnswerDraft(BaseModel):
    answer: str
    cited_evidence: list[GroundedGuideCitation] = Field(default_factory=list)
    # Retained only to safely parse cached provider output from before claim verification.
...
def response_from_grounded_draft(
    ...
    verified_citations = citations.verify_grounded_citations(
        search.output,
        draft.answer,
        [citation.model_dump() for citation in draft.cited_evidence],
    )
    cited_evidence_ids = [citation.source_id for citation in verified_citations]
    cited_chunk_ids = [citation.chunk_id for citation in verified_citations]
    ...
    if draft.cited_evidence and not cited_evidence_ids:
        unsupported = [
            *unsupported,
            "No citation included a retrieved quote that supports this answer.",
        ][:4]
